...
class Query:
    ...
    def __add_filters(self, criterion):
        ...
        for prop, operaion_value_pairs in criterion.items():
            ...
            self.__check_property(prop_name)
            ...
            for operation, value in operaion_value_pairs.items():
                ...
                filters = self.__filters.get(operation)
                if filters is None:
                    filters = []
                    self.__filters[operation] = filters
                    filters.append({
                        'name': prop_name,
                        'value': value
                    })
    ...
    def find(self, size = 100):
        aql_source, aql_filter, aql_bind = self.__build_aql_trio()
