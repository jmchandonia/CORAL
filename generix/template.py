...
def generate_brick_F3D_tabs_template(brick_ds, file_name):
    ...
    for sheet in sheets:
        ...
        dim3_vars = []
        for dim_var in brick_dims[2]['variables']:
            var_name = _get_contexted_var_name(dim_var)
            dim3_vars.append('%s:%s' % (brick_dims[2]['type']['text'], var_name))
        ...
        rows = [
            ...
            ['Dimension 1', dims[0]],
            ['Dimension 2', dims[1]],
            ['Dimension 3', brick_dims[2]['type']['text']],
            ['Data values', data_var],
            [],
        ...
        for i in dim3_vars:
           dim3_row_labels.append(i)
           dim3_rows.append('')

        rows.append(dim3_row_labels)
        rows.append(dim3_rows)
        ...
        sheet['A6'].fill = DIM3_FILL
        sheet['B6'].fill = DIM3_FILL

        sheet['A7'].fill = DATA_FILL
        sheet['B7'].fill = DATA_FILL

        sheet['A9'].font = INSTRUCTIONS_FONT
        sheet['A10'].font = INSTRUCTIONS_FONT
        ...
        # Color Dim 3
        start_row = len(rows) - 1
        for ci, _ in enumerate(dim3_vars):
            sheet.cell(row=start_row - 1, column=ci + start_column).fill = DIM3_FILL
            sheet.cell(row=start_row, column=ci + start_column).fill = DIM3_FILL
...
def generate_brick_FND_template(brick_ds, file_name, tab_data=False):
    TITLE_FONT = Font(name='Calibri', size=14)
    INSTRUCTIONS_FONT = Font(name='Calibri', size=10, bold=True, italic=True)
    DIM1_FILL = PatternFill('solid', fgColor='FFFFDD')
    DIM2_FILL = PatternFill('solid', fgColor='DDFFDD')
...
def generate_brick_FND_interlace_template(brick_ds, file_name):
    TITLE_FONT = Font(name='Calibri', size=14)
    INSTRUCTIONS_FONT = Font(name='Calibri', size=10, bold=True, italic=True)
    COLUMN_WIDTH = 18
    dim1_row_counts = 10
...
def parse_brick_FND_data(brick_ds, file_name):
    SEARCH_FORMAT_MAX_ROW = 100
    book = load_workbook(file_name)
    sheet = book.active
    ...
    if template_format == 'FND_INTERLACE':
        return parse_brick_FND_interlace_data(brick_ds, sheet)
    elif template_format == 'FND_TAB_DIM':
        return parse_brick_FND_tab_dim(brick_ds, book)
    elif template_format == 'FND_TAB_DATA':
        return parse_brick_FND_tab_data(brick_ds, book)
    else:
...
def parse_brick_FND_interlace_data(brick_ds, sheet):
...
def parse_brick_FND_tab_dim(brick_ds, book):
    dims_ds = brick_ds['dimensions']
